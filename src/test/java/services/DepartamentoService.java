...
public class DepartamentoService {
    ...
    public void setFieldsDepartamento(String campo, String valor) {
        if ("nome".equalsIgnoreCase(campo)) {
            if (valor == null || valor.isEmpty()) {
                departamentoModel.setNome("");
            } else {
                departamentoModel.setNome(valor);
            }
        }
    }

    public void createDepartamento(String endPoint) {
        Response response = given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + TestContext.TOKEN)
                .body(gson.toJson(this.departamentoModel))
                .when()
                .post(endPoint)
                .then()
                .extract().response();
        ...
        if (response.getStatusCode() == 201) {
            TestContext.LAST_CREATED_DEPARTAMENTO = gson.fromJson(response.getBody().asString(), DepartamentoModel.class);
        }
        TestContext.LATEST_RESPONSE = response;
    }

    public void deleteDepartamento(String endPoint) {
        String finalEndpoint = endPoint + "/" + TestContext.LAST_CREATED_DEPARTAMENTO.getId();
        Response response = given()
                .header("Authorization", "Bearer " + TestContext.TOKEN)
                .when()
